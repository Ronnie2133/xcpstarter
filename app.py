...
def batch_update_ingredient(batch_id, ing_id):
    ...
    if unit in ('g','kg','oz','lb','ml','l'):
        ing.unit = unit
    db.session.commit()
    flash('Batch ingredient updated.')
...
def batches_export_csv():
    si = io.StringIO()
    w = csv.writer(si)
    w.writerow(['batch_name','raw_qty','yield_qty','yield_unit','notes','component_type','component_name','qty','unit'])
    for b in BatchRecipe.query.order_by(BatchRecipe.name).all():
        for ing in b.ingredients:
            w.writerow([b.name, b.raw_qty or '', b.yield_qty, b.yield_unit, b.notes or '', 'inventory', ing.item.name, ing.qty, ing.unit])
        for sb in getattr(b, 'sub_batches', []):
            w.writerow([b.name, b.raw_qty or '', b.yield_qty, b.yield_unit, b.notes or '', 'batch', sb.child.name, sb.qty, sb.unit])
    return Response(
        si.getvalue(),
        mimetype='text/csv',
...
def batches_import_csv():
    ...
    for bname, items in groups.items():
        first = items[0]
        raw_qty_raw = (first.get('raw_qty') or '').strip()
        try:
            raw_qty = float(raw_qty_raw) if raw_qty_raw != '' else None
        except Exception:
            raw_qty = None
        try:
        ...
        if not batch:
            batch = BatchRecipe(name=bname, raw_qty=raw_qty, yield_qty=yield_qty, yield_unit=yield_unit, notes=notes)
            db.session.add(batch)
            db.session.flush()
            created += 1
        else:
            batch.raw_qty = raw_qty
        ...
        for r in items:
            ...
            if ctype == 'inventory':
                ...
                if not child:
                    child = BatchRecipe(name=cname, raw_qty=None, yield_qty=1.0, yield_unit=unit, notes='(placeholder)')
...
def menu_update_inv(menu_id, ing_id):
    ...
    if unit in ('g','kg','oz','lb','ml','l'):
        c.unit = unit
    db.session.commit()
    flash('Inventory component updated.')
