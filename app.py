...
class BatchIngredient(db.Model):
    ...
    def ext_cost(self) -> float:
        try:
            ...
            # Same unit (e.g. ct) — just multiply directly
            if self.item.unit.lower() == self.unit.lower():
                return self.item.unit_cost * self.qty
            return 0.0
    ...
    def ext_calories(self) -> float:
        ...
        if not self.item.calories_per_100g:
            return 0.0
        if self.unit.lower() == 'ct':
            # Can't convert count to grams — skip calories
            return 0.0
        try:
...
def inventory_import_csv():
    ...
    for row in csv.DictReader(io.StringIO(text)):
        ...
        if not name or unit not in ('g','kg','oz','lb','ml','l','ct'):
            continue
...
def batches():
    if request.method == 'POST':
        ...
        if not name or yield_unit not in ('g','kg','oz','lb','ml','l','ct'):
    ...
    if request.method == 'POST':
        ...
        for item_id, qty, unit in zip(item_ids, qtys, units):
            try:
                ...
                continue
            if unit not in ('g','kg','oz','lb','ml','l','ct') or qty <= 0:
                continue
...
def batches_import_csv():
    ...
    for bname, batch_rows in groups.items():
        ...
        try:
            ...
            prep_time_min = 0
        if yield_qty <= 0 or yield_unit not in ('g','kg','oz','lb','ml','l','ct'):
            skipped += 1; continue
        ...
        for r in batch_rows:
            ...
            if not cname or qty <= 0 or unit not in ('g','kg','oz','lb','ml','l','ct'):
                continue
            if ctype == 'inventory':
